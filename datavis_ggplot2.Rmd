...
- I personally like the `viridis` color schemes. *(These are not currently included in `ggplot2` itself, but will be in the next version released to CRAN this summer. You can install the `viridisLite` package to use them now. **ADD LINK**)* These scales print well, even in black & white, and are built to be perceived by people with color blindness.
...
As we saw, a legend will not be helpful here - there are too many countries.
